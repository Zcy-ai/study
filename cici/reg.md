...
* c: 更改匹配行内容
* g: 全局替换
* r: read a file and append it at the current point.
* w: write
...
# « filtre »  后加《 is genius》 
# &用于保存查找到的字符串以便在替换中使用
sed 's/filtre/& is genius/g' filepath
```
...
``` shell
sed 's/\([0-9][0-9]*\)/aa\1aa/g' filepath
```

``` shell
# 删除文件所有sujet开头的行
sed '/^sujet/d' filepath
```
...
* sed 's/ //g' file
* sed '^#/d' file
* sed 's/^\(.*\)\(.\)\(.\)$/\119\2\2\3/g' shenjiayi_sed
* 删除不含邮数字字母的行
* 
* 